...
<table style='width:100%;border:1px solid green;'>
	<tr>
	<td style='width:33%;text-align:center'><h3>메뉴관리</h3></td>
...
.on('click','#btnAdd',function(){
	...
	$.post('http://localhost:8082/addMenu',{name:$('#name').val(),price:$('#price').val()},
			function(rcv){
				console.log('before getList');
				getList();
	},'text');
	} else {	// update
		$.post('http://localhost:8082/updateMenu',{id:$('#id').val(), name:$('#name').val(),price:$('#price').val()},
			function(rcv){
				getList();
		}, 'text');
	}
...
.on('click','#btnDelete',function(){
	...
	$.post('http://localhost:8082/deleteMenu',{id:$('#id').val()}, function(rcv){
		getList();
	},'text');
	return false;
}) 
